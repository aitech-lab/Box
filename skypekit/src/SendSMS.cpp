...
int main(int argc, char * argv[]) {
  ...
  if (argc < 3) {
    printf("usage: SendSMS <skypename> <password> <sms folder>\n");
    return 0;
  };
  ...
  if (skype->GetAccount(myAccountName, account)) {
	...
	int timeout = 0;
        ...
        string phone = "";
        string sms_text   = "";
        getNextSms("./sms", phone, sms_text);
        
        // no sms file? wait and chekc again
        if(phone=="" || sms_text=="") {
            cout << "nothing to send..."<<"\n";
            Delay(1);
            continue;
        }
        cout << "Gog new sms for " << phone << "\n" << sms_text << "\n";
        targetList.clear();
        targetList.append(phone.c_str());
        ...
        SEStringList chunks;
        sms->SetBody(sms_text.c_str(), setBodyResult, chunks, freeUntilNextChunk);
        ...
        if (setBodyResult != Sms::BODY_OK) {
          ...
          if (!targetsSuccess) {
            ...
            while (!sms->canSend) { Delay(1); cout << "-"; };
            ...
            timeout = 0;
            while ( (!sms->isDelivered) && (!sms->hasFailed) && timeout<15 ) { Delay(1); timeout++; cout <<"."; };
